...
import io.github.sleepy_evelyn.territorial.api.sound.TerritorialSounds;
import io.github.sleepy_evelyn.territorial.block.CorruptedBeaconBlock;
...
import static io.github.sleepy_evelyn.territorial.api.sound.TerritorialSounds.CORRUPTED_BEACON_POWER_SELECT;
import static io.github.sleepy_evelyn.territorial.api.sound.TerritorialSounds.CORRUPTED_BEACON_AMBIENT;

public class
CorruptedBeaconBlockEntity extends BlockEntity implements NamedScreenHandlerFactory {
            ...
            if (block instanceof Stainable) {
                ...
                if (blockEntity.beamSegmentsTemp.size() <= 1) {
                    ...
                    if (Arrays.equals(colours, beamSegment.color)) {
                        ...
                        beamSegment = new BeamSegment(new float[]{(beamSegment.color[0] + colours[0]) / 2.0F, (beamSegment.color[1] + colours[1]) / 2.0F, (beamSegment.color[2] + colours[2]) / 2.0F});
                        blockEntity.beamSegmentsTemp.add(beamSegment);
                    }
                }
            } else {
