...
import io.github.sleepy_evelyn.territorial.mixin.common.ThreadedChunkManagerAccessor;
import io.github.sleepy_evelyn.territorial.util.MathUtils;
import io.github.sleepy_evelyn.territorial.util.NbtUtils;
...
public class LaserTransmitterBlockEntity extends BlockEntity {
    ...
    private static void beamTick(World world, BlockPos pos, BlockState state, LaserTransmitterBlockEntity be, List<Entity> entitiesCollided) {
        ...
        if (!world.isClient) {
            ...
            int watchDistance = ((ThreadedChunkManagerAccessor) ((ServerWorld) world).getChunkManager().delegate).getWatchDistance();
        ...
        for(var entity : entities) {
            if(targetAllMobs || entity.isPlayer()) {
                ...
                if(be.mods.get("death") && be.world != null)
                    entity.damage(TerritorialDamageSources.create(be.world, TerritorialDamageSources.LASER), 4.0F);
            }
        }
    }
