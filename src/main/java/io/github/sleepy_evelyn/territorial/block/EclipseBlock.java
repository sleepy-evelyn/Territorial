...
public interface EclipseBlock {
    ...
    default void eclipseDisplayTick(BlockState state, World world, BlockPos pos, RandomGenerator random, TickCounter ticker, int totalDuration, int maxReach) {
        if (random.nextBoolean() && ticker.test()) { // Random tick
            ...
            if(duration < totalDuration * 0.9D) { // Prevent packet spam
                if(!player.isCreative() && !player.isSpectator() && player.getWorld().getDimension().hasSkyLight()) {
